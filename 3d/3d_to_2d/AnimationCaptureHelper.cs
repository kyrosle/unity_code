...
public class AnimationCaptureHelper : MonoBehaviour
{
    ...
    /// <summary>
    /// The target gameobject camera pos  to capture.
    /// </summary>
    //[SerializeField]
    ...
    /// </summary>
    //[SerializeField]
    public AnimationClip _sourceClip ;
    ...
    public IEnumerator CaptureAnimation(Action<Texture2D, Texture2D> onComplete)
    {
        ...
        try
        {
            _targetpos = _target.transform.GetChild(1).gameObject;
            for (_currentFrame = 0; _currentFrame < numFrames; _currentFrame++)
            {
                // _captureCamera.transform.position = new Vector3(0f,-.06f, 0f)+_targetpos.transform.position;
